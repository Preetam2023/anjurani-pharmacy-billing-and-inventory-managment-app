...
from ui.widgets import SelectAllSpinBox, ExpiryLineEdit
...
class MedicineFormDialog(QDialog):
    ...
    self.name_completer_model,
    self,
)

        self.name_completer.setCaseSensitivity(
    Qt.CaseSensitivity.CaseInsensitive
)

        self.name_completer.setFilterMode(
    ...
    QCompleter.CompletionMode.PopupCompletion
        ...
        self.name_input.setCompleter(self.name_completer)
        self.name_completer.activated.connect(
    ...
    self.update_name_suggestions
        ...
        self.expiry_input = ExpiryLineEdit()
    ...
    def update_name_suggestions(self, text):
        """
    Show matching medicine names while typing.
    Only suggests existing medicines; does not prevent duplicates.
        """

        suggestions = queries.get_medicine_name_suggestions(text)

        display = []

        for med in suggestions:

            seller = med["seller_name"] or "-"

            display.append(
            f"{med['name']}    | Stock: {med['stock']} | Seller: {seller}"
        )

            self.name_completer_model.setStringList(display)
            
    def on_name_selected(self, text):
        """
    Completer displays extra information.
    Store only the medicine name.
    """

        medicine_name = text.split("|")[0].strip()

        self.name_input.blockSignals(True)
        self.name_input.setText(medicine_name)
