...
from reports.invoice_router import generate_invoice
...
class BillSavedDialog(QDialog):
    def __init__(self, parent, invoice_no, grand_total):
        ...
        self.print_btn = buttons.addButton(
            "🖨 Print",
            QDialogButtonBox.ButtonRole.AcceptRole
        )

        self.preview_btn = buttons.addButton(
    "👁 Preview PDF",
    QDialogButtonBox.ButtonRole.ActionRole
        ...
        self.print_btn.clicked.connect(self.print_clicked)
...
class BillingScreen(QWidget):
    ...
    def build_search_panel(self):
        ...
        self.results_table.setHorizontalHeaderLabels(["Name", "Batch No", "Exp. Date","Packet Price",
        "Unit Price", "Stock"])
    ...
    row_index,
    3,
    QTableWidgetItem(f"₹{med['packet_price']:.2f}")
)

            self.results_table.setItem(
    row_index,
    4,
    QTableWidgetItem(f"₹{med['price']:.2f}")
)

            self.results_table.setItem(
    row_index,
    5,
    QTableWidgetItem(str(med["stock"]))
)
    ...
    0,
    3,
    QTableWidgetItem(f"₹{medicine['packet_price']:.2f}")
)

        self.results_table.setItem(
    0,
    4,
    QTableWidgetItem(f"₹{medicine['price']:.2f}")
)

        self.results_table.setItem(
    0,
    5,
    QTableWidgetItem(str(medicine["stock"]))
)
    ...
    def print_pdf(self, pdf_path):
        ...
        try:
            ...
            if sys.platform.startswith("win"):
                sumatra = resource_path("assets/tools/SumatraPDF.exe")
                # print(sumatra)
                # print(os.path.exists(sumatra))
