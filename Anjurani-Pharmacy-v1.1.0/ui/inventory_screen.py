...
class InventoryScreen(QWidget):
    COLUMNS = ["Name", "Batch No", "Expiry Date", "Packet Price", "Unit Price", "Stock", "Seller"]
...
""")
        ...
        self.table.setColumnWidth(1, 110)   # Batch
        self.table.setColumnWidth(2, 110)   # Expiry
        self.table.setColumnWidth(3, 100)   # Packet Price

        self.table.setColumnWidth(4, 90)    # unit Price
        self.table.setColumnWidth(5, 70)    # Stock
        self.table.setColumnWidth(6, 130)   # Seller
    ...
    def load_data(self):
        ...
        medicines = (
        queries.search_medicines(query)
        if query
        else queries.get_all_medicines()
    )
        ...
        for row_index, med in enumerate(medicines):
            ...
            packet_price_item = NumericTableWidgetItem(f"₹{med['packet_price']:.2f}")
            packet_price_item.setData(
    Qt.ItemDataRole.UserRole,
    med["packet_price"]
)
            self.table.setItem(row_index, 3, packet_price_item)

            unit_price_item = NumericTableWidgetItem(f"₹{med['price']:.2f}")
            unit_price_item.setData(
                Qt.ItemDataRole.UserRole, med["price"]
)
            self.table.setItem(row_index, 4, unit_price_item)
            ...
            self.table.setItem(row_index, 5, stock_item)

            self.table.setItem(row_index, 6, QTableWidgetItem(med["seller_name"] or "—"))
        ...
        # print(f"Query Time      : {(t2-t1):.4f} sec")
        # print(f"Populate Table  : {(t3-t2):.4f} sec")
        # print(f"Finalize        : {(t4-t3):.4f} sec")
